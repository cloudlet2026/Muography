...
struct Line
{
    ...
    Point closestPointToLine(const Line &other) const
    {
        ...
        Double_t sc = (b * e - c * d) / denom;
        Double_t tc = (a * e - b * d) / denom;
...
void analyze_output(const char *inputFile = "build/output.csv")
{
    ...
    while (std::getline(inFile, line))
    {
        ...
        EventData data;
        char comma;
        ss >> data.eventId >> comma >> data.moduleId >> comma >> data.x >> comma >> data.y;
    ...
    for (size_t i = 0; i < pocaDistances.size(); i++)
    {
        ...
        if (pocaDistances[i] < minPocaDistance)
            minPocaDistance = pocaDistances[i];
        if (pocaDistances[i] > maxPocaDistance)
            maxPocaDistance = pocaDistances[i];
        if (scatteringAngles[i] < minScatteringAngle)
            minScatteringAngle = scatteringAngles[i];
        if (scatteringAngles[i] > maxScatteringAngle)
            maxScatteringAngle = scatteringAngles[i];
    }
